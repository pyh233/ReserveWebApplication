<template>
  ...
  <div class="search">
    ...
    <el-form :inline="true" :model="searchModel" ref="searchFromRef">
      ...
      <el-form-item label="房间名称:" prop="name">
        <el-input v-model="searchModel.name" placeholder="请输入房间名称" />
      </el-form-item>
    </el-form>
  </div>
  <!-- 按钮区 -->
  <div class="action">
    <el-button
      ...
      >新增</el-button
    >
    <el-button color="#626aef" :icon="Edit" @click="editRoomClick" plain
      >修改</el-button
    >
    ...
    <el-button color="#626aef" :icon="Refresh" @click="resetFrom" plain
      >重置</el-button
    >
    <el-button type="danger" :icon="Delete" @click="deleteRoomClick" plain
      >删除</el-button
    >
  </div>
  <!-- 数据显示区 -->
  <div class="data-grid">
    ...
    >
      ...
      <el-table-column prop="maxCount" label="最大容纳人数" width="170" />
    </el-table>
    <!-- 分页条组件 -->
    <el-pagination
      class="pi"
  ...
  >
    ...
    >
      ...
      <el-form-item label="maxCount" prop="maxCount">
        <el-input
          v-model="formModel.maxCount"
...
import { findAll, deleteRoomRequest, save, update } from "@/api/RoomApi";
...
async function deleteRooms(ids) {
  let resp = await deleteRoomRequest(ids);
  if (resp.success) {
    ElMessage({
      type: "success",
...
function addRoomClick() {
  ...
  dialogTitle.value = "Add Room";
}

// 修改对话框
function editRoomClick() {
  let rows = tableRef.value.getSelectionRows();
  if (rows.length > 1) {
    ElMessage({
      type: "warning",
...
async function doSubmit() {
  auformRef.value.validate(async (valid) => {
    if (valid) {
      let room = toRaw(formModel);
      if (room.id && room.id !== "") {
        let resp = await update(room);
        if (resp.success) {
          ElMessage({
            type: "success",
            message: resp.message,
